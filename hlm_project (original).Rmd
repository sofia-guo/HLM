...
- \usepackage{lipsum}
- \pagestyle{fancy}
- \fancyhead[CO,CE]{EDUC 275G Final Project}
...
#### Investigation proportion (percent):
...
#### Substantiation proportion (percent):
...
summarystats <- as.data.frame(relevantcounties[,c("LWratio","ChildPovRt","InvAlleg","SubInv","PercBiden","scaledworker", "scaledcase")])
...
overall_mean <- df_dev %>%
  summarize(mean(LWratio)) %>% pull()
  
nj_bar <- df_dev %>%
  group_by(EmployerCounty) %>%
  summarize(n_j = n()) %>% pull(n_j) %>% mean()
...
```{r,echo=FALSE, results='asis',fig.pos='H',fig.align='center', fig.height=4}
...
#view results
texreg(list(mod_lmer1,mod_lmer2), caption="Random intercept linear multilevel model regression results",caption.above = T, custom.coef.map=list("InvAlleg"="Investigation %","SubInv"="Substantiation %","ChildPovRt"="Child Poverty %","PercBiden"="Voted Biden %","scaledworker"="No. Workers/Agency","scaledcase"="No. Investigations/Worker"))
```
...
Concise summary and brief mention of implications/limitations provided.
